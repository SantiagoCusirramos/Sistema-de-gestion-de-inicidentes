...
hdr "Sistema"
sudo pacman -Sy --noconfirm &>/dev/null && ok "Repos actualizados"

hdr "Dependencias"
pkg_ok() {
    pacman -Qi "$1" &>/dev/null \
        && ok "$1 presente" \
        || { info "Instalando $1..."; sudo pacman -S --noconfirm "$1" &>/dev/null && ok "$1 instalado"; }
}
pkg_ok "jdk21-openjdk"
pkg_ok "maven"
pkg_ok "mysql"
...
for i in {1..20}; do
    sudo mariadb-admin ping --silent 2>/dev/null && break
    sleep 1
